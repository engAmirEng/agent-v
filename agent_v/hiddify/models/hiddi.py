...
class BoolConfig(models.Model):
    child = models.OneToOneField('Child', models.DO_NOTHING, primary_key=True)  # The composite primary key (child_id, key) found, that is not supported. The first column is selected.
...
class ShowDomain(models.Model):
    domain = models.OneToOneField(Domain, models.DO_NOTHING, primary_key=True)  # The composite primary key (domain_id, related_id) found, that is not supported. The first column is selected.
...
class ShowDomainParent(models.Model):
    domain = models.OneToOneField(ParentDomain, models.DO_NOTHING, primary_key=True)  # The composite primary key (domain_id, related_id) found, that is not supported. The first column is selected.
...
class User(models.Model):
    ...
    usage_limit_gb = models.TextField(db_column='usage_limit_GB')  # Field name made lowercase. This field type is a guess.
    package_days = models.IntegerField()
    mode = models.CharField()
    monthly = models.BooleanField(blank=True, null=True)
    start_date = models.DateField(blank=True, null=True)
    current_usage_gb = models.TextField(db_column='current_usage_GB')  # Field name made lowercase. This field type is a guess.
